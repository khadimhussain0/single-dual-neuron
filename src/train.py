...
def plot_confusion_matrix(cm, output_dir):
    ...
    # Add class labels (0 and 1 for binary classification)
...
def save_results_to_csv(results, output_dir):
    ...
    # Save also as pretty formatted markdown for easy viewing
...
def run_experiment(model_name, dataset, batch_size=32, epochs=30, img_size=(32, 32, 3), learning_rate=0.001):
    ...
    with open(report_path, 'w') as f:
        ...
        # Add AUC separately since it's in a different location
...
if __name__ == "__main__":
    # Run experiment for airplane vs automobile
    print("Starting binary classification research experiments...")
    
    # First experiment with Small CNN
    run_experiment(
        model_name='small_cnn',
        dataset=(0, 1),  # Airplane vs. Automobile
        batch_size=64,
        epochs=20  # Reduced epochs for faster experimentation
    )
    
    # Optionally, run additional experiments with different classes or models
    # For a comprehensive study, uncomment and run these:
    """
    # Different class pairs with the same model
    run_experiment(model_name='small_cnn', dataset=(3, 5))  # Cat vs Dog
    run_experiment(model_name='small_cnn', dataset=(2, 9))  # Bird vs Truck
    
    # Same classes with different models
    run_experiment(model_name='vgg16', dataset=(0, 1))     # Airplane vs Automobile with VGG16
    run_experiment(model_name='resnet50', dataset=(0, 1))  # Airplane vs Automobile with ResNet50
    """
